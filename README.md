...
Londoncoin allows many computers to behave as one. It uses Peer to Peer (P2P) 
...
For guidance on how to install and use Londoncoin please visit our 
[documentation](http://docs.londocin.io) pages.
...
Londoncoin projects the output of the consensus algorithm onto a linear blockchain 
...
Londoncoin is designed to integrate with applications written in any programming 
language.
...
Londoncoin is an ordering system that plugs into any application thanks to a very 
...
commands directly to the *state*, Londoncoin applications must forward the commands 
to Londoncoin and let them be processed asynchronously by the consensus system 
before receiving them back, in blocks, ready to be applied to the *state*.  

### API

Londoncoin communicates with the App through an `AppProxy` interface, which has two
implementations:
...
- `InmemProxy` : An InmemProxy uses native callback handlers to integrate Londoncoin 
                 as a regular Go dependency. 

The `AppProxy` interface exposes three methods for Londoncoin to call the App:
...
Reciprocally, `AppProxy` relays transactions from the App to Londoncoin via a native 
Go channel - `SubmitCh` - which ties into the application differently depending 
on the type of proxy (Socket or Inmem).

Londoncoin asynchronously processes transactions and eventually feeds them back to 
the App, in consensus order and bundled into blocks, with a **CommitBlock** 
call. Transactions are just raw bytes and Londoncoin does not need to know what they 
...
### Go
Londoncoin is written in [Golang](https://golang.org/). Hence, the first step is to 
...
```
Londoncoin uses [Glide](http://github.com/Masterminds/glide) to manage dependencies.
...
Londoncoin has extensive unit-testing. Use the Go tool to run tests:
```bash
[...]/babble$ make test
```
...
To see Londoncoin in action, we have provided a series of scripts to bootstrap a 
...
```
Indeed, each node is comprised of an App and a Londoncoin node (cf Design section).

Run the **demo** script to play with the **Dummy App** which is a simple chat 
application powered by the Londoncoin consensus platform:
